...
public class RmSignRoomServiceImpl implements RmSignRoomService {
	...
	public void addReservation(ReservationInfoVo reservationInfoVo) {
		...
		if (reservationInfoVo != null) {
			...
			reservation.setSigningCenterId(reservationInfoVo
					.getSigningCenterId());
			reservation.setNumberOfParticipants(reservationInfoVo
					.getNumberOfParticipants());
			reservation
					.setNumberOfPeople(reservationInfoVo.getNumberOfPeople());
			...
			reservation.setResPersonMobile(reservationInfoVo
					.getResPersonMobile());
	...
	public boolean isUsedByRmRoomSchedule(ReservationInfoVo reservationInfoVo) {
		RmRoomSchedule rmRoomSchedule = rmRoomScheduleMapper
				.getRmRoomScheduleByPkid(Long.valueOf(reservationInfoVo
						.getScheduleId()));
	...
	@Override
	public List<List<DateWeekVo>> showSchedulingData(Map map)
		...
		List<TradeCenterSchedule> tcss = tradeCenterScheduleMapper
				.queryTradeCenterSchedules(params);// 日历范围内的所有值班数据
	 ...
	 * @throws ParseException
	 */
	public List<DateWeekVo> getMonthSchedules(String date)
			throws ParseException {
		...
		c1.set(Calendar.DAY_OF_MONTH,
				c1.getActualMaximum(Calendar.DAY_OF_MONTH));// 指定日期月份的最后一天
		...
		if (firstDay > 1) {// 星期日外国定为1 取指定月份前一个月的最后几天
			...
			for (int i = headWeekNum - 1; i >= 0; i--) {
				...
				if (sdf.parse(sdf.format(c2.getTime())).getTime() >= sdf.parse(
						curDate).getTime()) {// 当日之后可编辑
					dw.setEdit(true);
				}
		...
		for (int j = 1; j <= midWeekNum; j++) {
			...
			if (sdf.parse(sdf.format(c1.getTime())).getTime() >= sdf.parse(
					curDate).getTime()) {// 当日之后可编辑
				dw.setEdit(true);
			}
			dw.setDay(Integer.valueOf(sdf1.format(c1.getTime())));
			if (sdf.format(c1.getTime()).equals(
					sdf.format(Calendar.getInstance().getTime()))) {// 当天高亮
				dw.setLight(true);
			}
			dates.add(dw);
		}
		if (ConstantsUtil.SHOW_DAYS > dates.size()) {// 取指定月份下一个月份的前几天数据
			...
			for (int i = 1; i <= lastWeekNum; i++) {
				...
				dw.setDay(Integer.valueOf(sdf1.format(c2.getTime())));
				if (sdf.format(c2.getTime()).equals(
						sdf.format(Calendar.getInstance().getTime()))) {// 当天高亮
					dw.setLight(true);
				}
				dates.add(dw);
	...
	@Override
	public int addOrUpdateTradeCenterSchedule(
			TradeCenterSchedule tradeCenterSchedule) {
		if (tradeCenterSchedule.isChanged()) {// 更新值班表
			return tradeCenterScheduleMapper
					.updateTradeCenterSchedule(tradeCenterSchedule);
		} else {// 新增值班信息
			return tradeCenterScheduleMapper
					.addTradeCenterSchedule(tradeCenterSchedule);
		}
	...
	public int deleteTradeCenterSchedule(TradeCenterSchedule tradeCenterSchedule) {
		return tradeCenterScheduleMapper
				.deleteTradeCenterSchedule(tradeCenterSchedule);
	}
	...
	public boolean isCurrenDayDuty() {
		...
		map.put("dutyDate", new SimpleDateFormat("yyyy-MM-dd").format(Calendar
				.getInstance().getTime()));
		List<TradeCenterSchedule> tcs = tradeCenterScheduleMapper
				.queryTradeCenterSchedules(map);
