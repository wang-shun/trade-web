...
public class RmSignRoomServiceImpl implements RmSignRoomService {
	...
	public boolean isUsedByRmRoomSchedule(ReservationInfoVo reservationInfoVo) {
		...
		if(rmRoomSchedule.getResStatus()==null){
			...
			return true;
		}else if(rmRoomSchedule.getResStatus()!=null && "5".equals(rmRoomSchedule.getResStatus().trim())){//提前使用中
	...
	public void generateSignRooms(List<RmSignRoom> signRooms,List<RmRoomSchedule> rmRoomSchedules,List<RmRoomSchedule> rrs){
			...
			for (RmSignRoom signRoom : signRooms) {
				...
				if(!rrs.isEmpty()){
					for(int i=0;i<rrs.size()-1;i++){
						...
						if("1".equals(rcprev.getUseStatus()) && curTime>endTime){//当状态为使用中时并且当前时间大于结束时间时
