...
public class TaskSubmitLogServiceImpl implements TaskSubmitLogService {
	...
	public void record(TaskVo task) {
		...
		SessionUser user = uamSessionService
				.getSessionUserById("8a8493d45921d56d01593fd036e100f1");
		...
		if (extProps != null) {
			...
			if (extProps.get("managerId") != null) {
				taskSubmitLog
						.setManagerId(extProps.get("managerId").toString());
			}
			if (extProps.get("districtId") != null) {
				taskSubmitLog.setDistrictId(extProps.get("districtId")
						.toString());
			}
			if (extProps.get("directorId") != null) {
				taskSubmitLog.setDirectorId(extProps.get("directorId")
						.toString());
			}
			...
			if (extProps.get("generalManagerId") != null) {
				taskSubmitLog.setGeneralManagerId(extProps.get(
						"generalManagerId").toString());
			}
			if (extProps.get("seniorManagerId") != null) {
				taskSubmitLog.setSeniorManagerId(extProps
						.get("seniorManagerId").toString());
			}
		}
