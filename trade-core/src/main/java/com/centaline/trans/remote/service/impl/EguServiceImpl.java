...
public class EguServiceImpl implements EguService {
    ...
    Logger                               logger         = LoggerFactory
        .getLogger(EguServiceImpl.class);
    ...
    public String saveAttachment(MortgageAttamentVo mortgageAttament) {
        ...
        if (CollectionUtils.isNotEmpty(preFileAdress)) {
            ...
            for (int i = 0; i < mortgageAttament.getPreFileAdress().size(); i++) {
                ...
                attachment.setFileCat(mortgageAttament.getPicName().get(i)
                    .substring(mortgageAttament.getPicName().get(i).indexOf(".") + 1));
                toAttachmentService.saveToAttachment(attachment);
            }
            List<ToAttachment> deleteAttachmentList = getDeletedAttachment(attachmentIds,
                mortgageAttament.getCaseCode());
    ...
    public String getAttachmentInfo(MortgageAttamentVo mortgageAttament) {
        ...
        if (mortgageAttament != null) {
            ...
            List<ToAttachment> attachmentList = getDeletedAttachment(attachmentIds,
                mortgageAttament.getCaseCode());
            if (CollectionUtils.isNotEmpty(attachmentList)) {
    ...
    public void assess(HouseInfoVo houseInfo) throws JsonParseException, JsonMappingException,
        ...
        Map<String, String> paramMap = obj.readValue(JSONObject.toJSONString(houseInfo),
        ...
        try {
            ...
            apiLogService.apiLog(EGU_MODULE, "/assess", url, ExceptionUtils.getStackTrace(e), "0",
                e.getMessage());
            throw e;
        }
        ...
        BaseResult<PricingResultVo> result = mapper.readValue(returnStr,
            new TypeReference<BaseResult<PricingResultVo>>() {
            });
    ...
    @Override
    public void disagree(DisagreeApplyVo disagreeApply) throws ClientProtocolException,
                                                        IOException {
        ...
        ObjectMapper obj = new ObjectMapper();
        Map<String, String> paramMap = obj.readValue(JSONObject.toJSONString(disagreeApply),
            new TypeReference<Map<String, String>>() {
            });
        paramMap.put("timestamp", String.valueOf(new Date().getTime()));
        paramMap.put("nonce", String.valueOf(new Random().nextInt(9000) + 1000));
    ...
    @Override
    public void confirm(PricingConfirmVo pricingConfirm) throws JsonParseException,
        ...
        ToEguPricing pricing = toEguPricingService
            .findIsFinalEguPricing(toEguPricing.getCaseCode());
        if (pricing == null) {
        ...
        BaseResult<PricingConfirmResultVo> result = mapper.readValue(returnStr,
            new TypeReference<BaseResult<PricingConfirmResultVo>>() {
            });
    ...
    private void upload(String caseCode, String evaCode,
                        MortgageAttamentVo mortgageAttament) throws ClientProtocolException,
                                                             IOException {
        ...
        if (CollectionUtils.isNotEmpty(mortgageAttament.getPreFileAdress())
            || (CollectionUtils.isNotEmpty(attList)
                && attList.size() != mortgageAttament.getAttachmentIds().size())) {
            ...
            paramMap.put("files",
                new BASE64Encoder().encode(files.getBytes("UTF-8")).replace(LINE_SEPERATOR, ""));

            paramMap = SignUtil.pathFilter(paramMap);

            String token = SignUtil.buildRequestToken(paramMap, Const.TOKEN);
            String url = evaCode + "/upload?token=" + token + "&"
                         + SignUtil.createLinkString(paramMap);
            HttpResponse httpResponse = executeGet(url);
            ...
            ObjectMapper mapper = new ObjectMapper();
            BaseResult<UploadFileResultVo> result = mapper.readValue(returnStr,
                new TypeReference<BaseResult<UploadFileResultVo>>() {
                });
            //失败
    ...
    @Override
    public void prereport(ToEvaReportVo evaReport,
                          MortgageAttamentVo mortgageAttament) throws JsonParseException,
                                                               JsonMappingException, ParseException,
                                                               IOException {
        String code = evaReport.getEvaCode();
        SessionUser user = uamSessionService.getSessionUser();
        List<ToEguPricing> toPricingList = toEguPricingService
            .findToEguPricingByEvaCode(evaReport.getEvaCode());
        ToEguPricing toPricing = toPricingList.get(0);
        if (StringUtils.isEmpty(toPricing.getApplyCode())) {
        ...
        ToEvaFeeRecord toEvaFeeRecord = toEvaFeeRecordService
            .findToEvaFeeRecordByCaseCode(evaReport.getCaseCode());
    ...
    @Override
    public void inquiryreport(ToEvaReportVo evaReport,
                              MortgageAttamentVo mortgageAttament) throws JsonParseException,
        ...
        paramMap = SignUtil.pathFilter(paramMap);
        String token = SignUtil.buildRequestToken(paramMap, Const.TOKEN);
        String url = code + "/inquiryreport?token=" + token + "&"
                     + SignUtil.createLinkString(paramMap);
        HttpResponse httpResponse = executeGet(url);
        HttpEntity entity = httpResponse.getEntity();
        String returnStr = EntityUtils.toString(entity, "UTF-8");
    ...
    @Override
    public void report(ToEvaReportVo evaReport, MortgageAttamentVo mortgageAttament)
        ...
        TgGuestInfo tgGuestInfo = tgGuestInfoService
            .findTgGuestInfoById(Long.parseLong(toMortgage.getCustCode()));
        if (tgGuestInfo == null) {
    ...
    @Override
    public void saveEguBank(BankSearchVo bankSearch) throws JsonParseException,
                                                     JsonMappingException, ParseException,
                                                     IOException {
        SessionUser user = uamSessionService.getSessionUser();
        ...
        if (CollectionUtils.isNotEmpty(result.getResult())) {
            for (BankInfo bankInfo : result.getResult()) {
                ...
                for (BankInfo bi : result.getResult()) {
                    ...
                    List<TsBankEvaRelationship> bankList = tsBankEvaRelationshipService
                        .findByBankCode(bi.getId());
