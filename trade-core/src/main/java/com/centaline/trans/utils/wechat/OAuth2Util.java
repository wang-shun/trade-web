...
public class OAuth2Util {
	...
	public static String GetCode() {
		...
		get_code_url = GET_CODE.replace("CORPID", ParamesAPI.corpId).replace(
				"REDIRECT_URI",
				CommonUtil.urlEncodeUTF8(ParamesAPI.REDIRECT_URI));

		log.info("---------------------Start wechat param api------------------------------------------------------------");
		log.info(get_code_url);
		log.info("---------------------END wechat param api------------------------------------------------------------");
		return get_code_url;
	}
	/**
	 * 企业如果需要员工在跳转到企业网页时带上员工的身份信息，需构造如下的链接
	 ...
	 *        redirect_uri/?code=CODE&state=STATE，企业可根据code参数获得员工的userid
	 * */
	public static String GetCode(String redirectUri) {
		String get_code_url = "";
		get_code_url = GET_CODE.replace("CORPID", ParamesAPI.corpId).replace(
				"REDIRECT_URI",
				CommonUtil.urlEncodeUTF8(redirectUri));
	...
	public static String GetUserID(String access_token, String code,
		...
		String UserId = "";
		CODE_TO_USERINFO = CODE_TO_USERINFO
				.replace("ACCESS_TOKEN", access_token).replace("CODE", code)
				.replace("AGENTID", agentid);
		log.info("in GetUserId---------- access_token:" + access_token
				+ "code:" + code + "agentid:" + agentid);
		JSONObject jsonobject = CommonUtil.httpsRequest(CODE_TO_USERINFO,
				"GET", null);
		if (null != jsonobject) {
			try {
				UserId = jsonobject.getString("UserId");
				if (!"".equals(UserId)) {
					log.info("获取UserId成功 UserId is :[" + UserId + "]");
				}
