...
ou.fullname AS deptName,p.ID AS PositionID,p.NAME AS positionName
 ...
 INNER JOIN dbo.ORG_Position_Employee pe ON emp.em_ID = pe.emid
 INNER JOIN dbo.ORG_Position p ON pe.positionID = p.ID
 INNER JOIN dbo.ORG_Unit ou ON p.UnitID = ou.ID
