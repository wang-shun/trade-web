package com.centaline.scheduler.sync.bj.service;
...
@Service("bJSyncService")
public class BJSyncServiceImpl extends SyncHrService {
	@Autowired
	@Qualifier("BJHrTemplate")
	...
	private static CityType city = CityType.BEIJING;
	private Logger logger = LoggerFactory.getLogger(BJSyncServiceImpl.class);

	@Override
	protected List<SyncOrgUnit> getSyncOrgUnit() {
		if (StringUtils.isBlank(orgQuerySql)) {
			ClassLoader classLoader = getClass().getClassLoader();
			URL url = classLoader.getResource("syncQuery/BJ/org.sql");
	...
	protected List<SyncEmployee> getSyncEmployee() {
		if (StringUtils.isBlank(employeeQuerySql)) {
			ClassLoader classLoader = getClass().getClassLoader();
			URL url = classLoader.getResource("syncQuery/BJ/employee.sql");
	...
	protected List<SyncPosition> getSyncPosition() {
		if (StringUtils.isBlank(positionQuerySql)) {
			ClassLoader classLoader = getClass().getClassLoader();
			URL url = classLoader.getResource("syncQuery/BJ/position.sql");
	...
	protected List<SyncEmpOrgPos> getSyncEmpOrgPos() {
		if (StringUtils.isBlank(empOrgPosQuerySql)) {
			ClassLoader classLoader = getClass().getClassLoader();
			URL url = classLoader.getResource("syncQuery/BJ/emp_org_pos.sql");
