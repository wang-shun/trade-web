...
public class CaseDetailController {
	...
	public String caseDetail(Long caseId, ServletRequest request){
		if(caseId==null)return "case/caseList";
		...
		for(String sp:tgproList){
			if(StringUtils.isEmpty(sp))continue;
		...
		if(toMortgage!=null){
			...
			if(null !=toMortgage.getCustCode()){
				/*for(TgGuestInfo guest:guestList){
					if(guest.getTransPosition().equals(TransPositionEnum.TKHXJ.getCode())){
    	...
    	if(caseInfo.getHouseProperty()!=null){				
    		String houseProperty = uamBasedataService.getDictValue(TransDictEnum.TFWXZ.getCode(), caseInfo.getHouseProperty());
    	...
    	if(caseInfo.getIsUniqueHome()!=null){				
    		String isUniqueHome = uamBasedataService.getDictValue(TransDictEnum.TWYZF.getCode(), caseInfo.getIsUniqueHome());
    		...
    		if(cashFlows!=null && cashFlows.size()!=0){
    			for(ToCashFlow toCashFlow:cashFlows){
    				...
    				if(!StringUtils.isEmpty(flowTypeCode)){
        				String flowTypeValue = uamBasedataService.getDictValue(TransDictEnum.TFWBM.getCode(), flowTypeCode);
    				...
    				if(toCashFlow.getFlowDirection()!=null){
    	    			String flowDirection = uamBasedataService.getDictValue(TransDictEnum.TLSFX.getCode(), toCashFlow.getFlowDirection());
    	...
    	if(toLoanAgents.size() >0){
    		for(ToLoanAgent toLoanAgent:toLoanAgents){
    			...
    			if(!StringUtils.isEmpty(toLoanAgent.getLoanSrvCode())){
    				String loanSrvName = uamBasedataService.getDictValue(TransDictEnum.TFWBM.getCode(), toLoanAgent.getLoanSrvCode());
    			...
    			if(!StringUtils.isEmpty(toLoanAgent.getApplyStatus())){
    				String applyStatusName = uamBasedataService.getDictValue(TransDictEnum.TSQZT.getCode(), toLoanAgent.getApplyStatus());
    				toLoanAgentVO.setApplyStatusName(applyStatusName);
    			}
    			//确认状态
    			if(!StringUtils.isEmpty(toLoanAgent.getConfirmStatus())){
    				String confirmStatusName = uamBasedataService.getDictValue(TransDictEnum.TSQZT.getCode(), toLoanAgent.getConfirmStatus());
	...
	public AjaxResponse<?> getSrvsByCaseCode(String caseCode, ServletRequest request){
		...
		//服务项
		StringBuffer srvCodes = new StringBuffer();
		List<TgServItemAndProcessor> tgSrvList = tgServItemAndProcessorService.findTgServItemAndProcessorByCaseCode(caseCode);
	...
	public AjaxResponse<?>  changeLeadingUser(String instCode,String caseCode ,String userId,HttpServletRequest request) {
		...
		/*List<TgServItemAndProcessor>tgservs =tgServItemAndProcessorService.findTgServItemAndProcessorByUserIdAndCaseCode(origUserId,caseCode);*/
...
//			}
	...
	public AjaxResponse<?>  startCasePrairses(String caseCode,String[] prItems ,HttpServletRequest request) {
		...
		if(reInt == 0)return AjaxResponse.fail("产调表更新失败！");
   ...
   @ResponseBody
   	public AjaxResponse<?>  saveSrvItems(String caseCode,String[] prItems, Boolean isDel, Boolean isRes, HttpServletRequest request) {
    	...
    	if(isDel || isRes ||srvs.size()>0){
    		...
    		//更新本地数据
        	StartProcessInstanceVo pIVo = workFlowManager.startCaseWorkFlow(process, sessionUser.getUsername(),caseCode);
			...
			for(String s: prItems){
				...
				for(Dict dictSon:listD) {
					...
					if(tgServItemAndProcessorService.findTgServItemAndProcessor(record) == null) {
			    		...
			    		if(reInsert == 0)return AjaxResponse.fail("服务与经办人表更新失败！");
					}
				}
	       	}
	       	return AjaxResponse.success("变更成功！");
   ...
   @ResponseBody
   public ToCaseInfoCountVo queryConutCaseByDate(Model model, ServletRequest request,
		   String createTime,String userId){
	    int jds =  0 ;
	   	int qys =  0 ;
	   	int ghs =  0 ;
	   	int jas =  0 ;
...
//		}
	   ...
	   if(TransJobs.TZJL.getCode().equals(sesseionUser.getServiceJobCode())){//总部
		  ...
		  // toCaseInfoCount = getToCaseInfoCountByOrgId(sesseionUser.getServiceOrgId());
		   if(orgList.size()>0){
			   for (Org org : orgList) {
    ...
    @ResponseBody
  	public AjaxResponse<?>  savePlanItems(String[] isChanges,String[] estIds,String[] estDates,String[] whyChanges,HttpServletRequest request) {
    	SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd");
    	SessionUser user = uamSessionService.getSessionUser();
    	for(int i=0;i<isChanges.length;i++){
    		if(isChanges[i].equals("true")){
    			...
    			if(oldPlan==null||oldPlan.getPkid() == null)return AjaxResponse.fail("未找到交易计划！");
    			...
    			if(reInt1==0)return AjaxResponse.fail("交易计划历史记录更新失败！");
    			int reInt = toTransPlanService.updateByPrimaryKeySelective(record);
    			if(reInt==0)return AjaxResponse.fail("交易计划更新失败！");
    		}
    	}
