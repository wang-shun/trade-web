...
public class WorkSpaceController
{
    ...
    public String showWorkSpace2(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        else if (TransJobs.TZJ.getCode().equals(jobCode))
        { // 总监
            /* 各个组织 */
    ...
    public Map workloadManagerBackoffice(WorkSpace work)
    {
        ...
        if (list != null && !list.isEmpty())
        {
            for (WorkLoad workLoad : list)
            {
                ...
                if (workLoad.gettCount() != null && workLoad.gettCount().intValue() != 0)
                {
                    String tCountStr = workLoad.getName() + ":" + workLoad.gettCount();
                    ...
                    {
                        wl.settCountStr(wl.gettCountStr() + "&nbsp;&nbsp;" + tCountStr);
                    }
                ...
                if (workLoad.getyCount() != null && workLoad.getyCount().intValue() != 0)
                {
                    String yCountStr = workLoad.getName() + ":" + workLoad.getyCount();
                    ...
                    {
                        wl.setyCountStr(wl.getyCountStr() + "&nbsp;&nbsp;" + yCountStr);
                    }
     ...
     * @date 2015/12/1
     */
    private boolean isCache(HttpServletRequest request, HttpServletResponse response)
    {
    ...
    public AjaxResponse cacheRemain(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException
    {
    ...
    public String showWorkSpaceMoblie(Model model, HttpServletRequest request) throws IOException
    {
        SessionUser user = uamSessionService.getSessionUser();
        ...
        {// 如果是总经理
            ...
            if (CollectionUtils.isNotEmpty(orgList))
            {
                for (Org org : orgList)
                {
                    userList = uamUserOrgService.getUserByOrgIdAndJobCode(org.getId(), TransJobs.TZJ.getCode());
                    for (User users : userList)
                    {
                        User u = new User();
                        u.setId(users.getId());
        ...
        { // 如果是总监
            ...
            if (CollectionUtils.isNotEmpty(orgList))
            {
                for (Org org : orgList)
                {
                    ...
                    // 获取全部的统计数据
                    toCaseInfoCount = getToCaseInfoCountByOrgId(org.getId().toString());
                    jds += toCaseInfoCount.getCountJDS();
                    qys += toCaseInfoCount.getCountQYS();
                    ghs += toCaseInfoCount.getCountGHS();
                    jas += toCaseInfoCount.getCountJAS();
    ...
    // 跟进userid查询统计数据
    private ToCaseInfoCountVo getToCaseInfoCount(String userId)
    {// 查询统计 接单
     // 接单数
    ...
    public Map trafficLightTips(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        {// 如果不是交易顾问
            ...
            transferOverdueCaseCount = benchBizwarnCaseCountQuery(currentUser.getServiceDepId(), "TransferOverdue", currentUser.getServiceDepHierarchy(),
                    "NO_CONSULTANT");
            loanerOverdueAcceptCount = benchBizwarnCaseCountQuery(currentUser.getServiceDepId(), "LoanerOverdueAccept", currentUser.getServiceDepHierarchy(),
                    "NO_CONSULTANT");
        }
        else
        {
            ...
            loanerOverdueAcceptCount = benchBizwarnCaseCountQuery(currentUser.getUsername(), "LoanerOverdueAccept", currentUser.getServiceDepHierarchy(),
                    "YES_CONSULTANT");
        }
        ...
        int curOverdueCaseEffCount = getCurOverdueCaseEffCount(currentUser.getId(), currentUser.getServiceJobCode(), serviceDepId);

        map.put("curOverdueCaseEffCount", curOverdueCaseEffCount);
     ...
     * @return 当前逾期案件时效数量
     */
    private int getCurOverdueCaseEffCount(String currentUserId, String jobCode, String oId)
    {
        JQGridParam gp = new CacheGridParam();
        gp.setPagination(false);
        gp.put("uid", currentUserId);
        gp.put("jobCode", jobCode);
        gp.put("oId", oId);
        gp.put("overdue", "curOverdued");
        gp.setQueryId("queryCaseEfficientList");

        Page<Map<String, Object>> pageCaseEff = quickGridService.findPageForSqlServer(gp);

        int curOverdueCaseEffCount = 0;
        if (pageCaseEff != null && pageCaseEff.getContent() != null && pageCaseEff.getContent().size() > 0)
        {
            curOverdueCaseEffCount = pageCaseEff.getContent().size();
        }

        return curOverdueCaseEffCount;
    ...
    private Map doSta(String serachId, String mo)
    {
        ...
        m.put("staLoanApply", JSONArray.toJSON(workSpaceService.staLoanApply(work)));
        m.put("staLoanSign", JSONArray.toJSON(setStaItemStr(workSpaceService.staLoanSign(work))));
        m.put("staLoanRelease", JSONArray.toJSON(workSpaceService.staLoanRelease(work)));
        return m;
    }
    ...
    private List<LoanStaDetails> setStaItemStr(List<LoanStaDetails> list)
    {
        if (list != null)
        {
            ...
            {
                loanStaDetails.setStaItemStr(uamBasedataService.findDictByTypeAndCode("yu_serv_cat_code_tree", loanStaDetails.getStaItem()).getName());
            }
        }
        return list;
    }
