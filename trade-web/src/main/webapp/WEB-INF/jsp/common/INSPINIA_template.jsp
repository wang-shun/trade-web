            ...
            <div class="sidebar-collapse">
                <ul class="nav" id="side-menu">
                   ...
                   <c:forEach items="${menuVO}" var="l1_menu" >
						<li class="">
								<c:choose><c:when test='${l1_menu.type=="MEN"}'><a href="${ctx}${l1_menu.absoluteUrl}" target="${l1_menu.target}" ><i class="fa ${l1_menu.icon}"></i> <span class="nav-label">${l1_menu.menuName }</span> <span class="fa arrow"></span></a></c:when>
									...
									<c:when test='${l1_menu.type=="MDL"}'>
										<ul class="nav nav-second-level">
											...
											<c:if test='${not empty l2_menu.resource.resourceCode}'>
													<li><a href="${ctx}${l2_menu.absoluteUrl}?<%=new Date().getTime() %>" target="${l1_menu.target}" ><i class="fa ${l2_menu.icon}"></i>
