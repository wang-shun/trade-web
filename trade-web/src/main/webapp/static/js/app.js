...
function activeBar(){
	...
	//查找去掉老的选中的side bar 的选中状态
	var activeOld = $("li[class='start active ']");
    var activeOldSpan = $("span[class='selected']");
    activeOld.removeAttr("class");
    activeOldSpan.remove();
    ...
    var subMenu = objs.parent().parent().attr("class");
    if("sub-menu" == subMenu){
    	sidebarMenu = objs.parent().parent().parent();
    	var activeNewLi = objs.parent();
    	activeNewLi.attr("class","active")
    }else{
    	sidebarMenu = objs.parent();
    }
    
    //处理相应的DOM,加上相应属性，改为已经选中
    sidebarMenu.attr("class","start active ");
    var activeNewa = sidebarMenu.find("a").get(0);
    var spanStr = '<span class="selected"></span>';
    $(activeNewa).append(spanStr);
}