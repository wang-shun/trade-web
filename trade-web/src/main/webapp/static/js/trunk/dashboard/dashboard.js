    ...
    function setStaValItem(d,c){
    	if(d){
    		$(d).each(function (i){
    			var _this=this;
    			var showhtml="金额："+_this.amount+"<br>单数："+_this.count+"<br>转化率："+_this.convRate+"%";
    ...
    function toDonutData(d,it){
    	...
    	if(d){
    		$(d).each(function(i){
    			dd.push({label:this['staItemStr'],value:$.unformatMoney(this[it]+"")});
    		});
    	}
    	return dd;
    }
   ...
   $(document).ready(function() {
		new messageGrid().init({e:$("#div_messagelist1"),u:appCtx['aist-message-web'],c:'MSG_YU_RESPONSE',ctx:ctx});
		new messageGrid().init({e:$("#div_messagelist2"),u:appCtx['aist-message-web'],c:'MSG_YU_WORK',ctx:ctx});
		new messageGrid().init({e:$("#div_messagelist3"),u:appCtx['aist-message-web'],c:'MSG_YU_STOPLOSS',ctx:ctx});
	        $('.i-checks').iCheck({
   ...
   function reloadMonth(){
	   var month = new Date().getMonth();
		var maxMonths = new Array("一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月");
