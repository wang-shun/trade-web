...
function ajaxDeleteInfo(pkid, index) {
    window.wxc.confirm("删除后信息无法恢复，确认删除？", {
        "wxcOk": function () {
            $.ajax({
                async: false,
                type: "POST",
 ...
 * @param calBak
 */
function loadReportInfo(reportCode,calBak) {
    $.ajax({
        type: 'GET',
        cache: false,
        url: ctx + '/task/bankRebate/reportInfo?reportCode=' + reportCode,
        dataType: "json",
        beforeSend: function () {
            $.blockUI({
                message: $("#salesLoading"),
        ...
        success: function (data) {
            $.unblockUI();
            if (data.success) {
                var info = data.bfiModel;
                info.count = info.dkCertMoney + info.ghCertMoney + info.bizMoney;
                new Vue({
                    el:'#reportInfo',
                    data:info
                });
 ...
 * @param String
 */
function showReportApprove(reportCode){
    $.ajax({
        type: 'GET',
        cache: false,
        url: ctx + '/bankRebate/showReportApprove?reportCode=' + reportCode,
        dataType: "json",
        beforeSend: function () {
            $.blockUI({
                message: $("#salesLoading"),
        ...
        success: function (data) {
            ...
            if (data.totalElements != 0) {
                ...
                var text = "审批人:"+info.OPERATOR+"<br/>";
                text += "审批时间:"+info.OPERATOR_TIME+"<br/>";
                text += "处理信息("+info.CONTENT+")";
                window.wxc.alert(text);
            } else {
                window.wxc.alert("未获取到审批记录！");
            }
        },
        error: function (errors) {
            $.unblockUI();
