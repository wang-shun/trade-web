 ...
 * 同赎楼案件任务列表
 */
function ransomTaskCheck(){
	setTimeout('initGrid()',1000); 
}

function initGrid() {
	var ransomCode = $('#ransomCode').val();
	var ransomProcessId = $('#PROCESS_DEFINITION_ID_RANSOM').val();
//	var ransomProcessId = 'RansomProcess:3:1235354';
	...
	$("#table_list_case_task").jqGrid(
			{
				...
				rowNum : 5,
				/* rowList: [10, 20, 30], */
				colNames : [ 'TASKID','RANSOMCODE','PARTCODE','INSTCODE', '任务名', '预计执行时间' ],
				colModel : [ {
					name : 'ID',
					index : 'ID',
					align : "center",
...
//					alert(url);
				...
				postData : {
					queryId : "queryRansomTaskListItemList",
					search_ransomCode:ransomCode,
					search_PROCESS_DEFINITION_ID_RANSOM:ransomProcessId
				}
