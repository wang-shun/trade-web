...
var DashBoard = function () {
    ...
    return {
        ...
        initMoneyGrid: function(ctx){
            jQuery("#moneyGrid")
		.jqGrid(
				{
					url : ctx+'/api/dashboard/topMoney/group', 
					datatype : "json",
					colNames : [ '业务员', '业绩','店组'],  
					colModel : [ {
						name : 'userName',
            ...
            $("#moneyGrid").setGridWidth($('#moneyGridDiv').width());

            function renderUser(cellvalue, options, rowObject) {
            	var userTemp = '';
            	userTemp +=	'<img class="mini-user-pic" src="'+ctx+'/media/image/avatar'+rowObject.sort+'.jpg">';
            	if (rowObject.sort <=3) {
            	userTemp +=	'<span class="btn red mini">' + rowObject.sort +'</span>';
            	}
            	
            	userTemp += '<a class="user-name">'+cellvalue+'</div>'; 
