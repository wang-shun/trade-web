...
(function($){
	...
	window.wxc.info = function(message,params,typeEnum){
		...
		if(isAutoHide){
			var showTime = (!settings.showTime)?2000:showTime;
			setTimeout(function(){
				$(".xcConfirm").remove();
			},showTime);
		}
	...
	window.wxc.xcConfirm = function(popHtml, type, options) {
		...
		var $popTip = $("<div>").addClass("popTip").attr("id","popup");//弹窗盒子
		var $ttBox = $("<div>").addClass("ttBox");//弹窗顶部区域
		var $txtBox = $("<div>").addClass("txtBox");//弹窗内容主体区
		var $btnArea = $("<div>").addClass("btnArea");//按钮区域

		var $ok = $("<a>").addClass("sgBtn").addClass("ok").attr("id","confirm").text("确 定");//确定按钮
		var $cancel = $("<a>").addClass("sgBtn").addClass("cancel").attr("id","cancel").text("取 消");//取消按钮
		var $input = $("<input>").addClass("inputBox");//输入框
		var $clsBtn = $("<a>").addClass("clsBtn").attr("id","close");//关闭按钮
