...
function renderMortgageComplete(f,data){
	var $content=data.content;
		if($content){
			...
			f.find('input[name=comYear]').val($content.comYear).attr("readonly",true);
			...
			f.find('input[name=comDiscount]').val($content.comDiscount).attr("readonly",true);
			//f.find('input[name=priCreditUnit]').val($content.priCreditUnit);
	...
	//showFinishButtonAlways:false,
 	...
 	onStepChanging: function (event, currentIndex, newIndex){
 		if(currentIndex == 0){
 			...
 			var option = [];
 			option.container = "comAndPSFLoanProcessfileUploadContainer";
