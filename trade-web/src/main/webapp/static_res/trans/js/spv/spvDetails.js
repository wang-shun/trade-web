...
function getAtr(i) {
	$str = '';
	$str += "<tr align='center'>";
	$str += "<td class='text-left'><select class='table-select'><option name='toSpvDeDetailList["
			+ sum + "].deCondCode' value=''>买方贷款审批完成</option></select></td>";
	$str += "<td class='text-left'><select class='table-select'><option name='toSpvDeDetailList["
			+ sum
			+ "].payeeAccountId' value='1'>资金方</option><option name='toSpvDeDetailList["
			+ sum + "].payeeAccountId' value='2'>卖方</option></select></td>";

	$str += "<td><input name='toSpvDeDetailList["
			+ sum
			+ "].deAmount' class='table-input-one' type='text' placeholder='请输入金额'>万</td>";
	$str += "<td class='text-left' ><input name='toSpvDeDetailList["
			+ sum
			+ "].deAddition' class='table-input' type='text' placeholder='' /></td>";
...
function checkFormSave() {
	...
	/*
	 * var toSpvAmount = $("#toSpvAmount").val(); if(toSpvAmount == null ||
	 * toSpvAmount == ''){ alert("请填写监管总金额"); return false; } var toSpvSpvInsti =
	 * $("#toSpvSpvInsti").val(); if(toSpvSpvInsti == null || toSpvSpvInsti ==
	 * ''){ alert("请填写监管产品"); return false; }
	 */
	return true;
}
...
function checkFormSubmit() {
	...
	if ((buyerGender == null || buyerGender == '')
			|| (sellerGender == null || sellerGender == '')) {
		alert("请勾选买/卖方性别！");
		return false;
	}
	...
	var buyerIdType = $("select[name='spvCustList[0].idType']").val();
	var sellerIdType = $("select[name='spvCustList[1].idType']").val();
	if ((buyerIdType == null || buyerIdType == '')
			|| (sellerIdType == null || sellerIdType == '')) {
		alert("请填写买/卖方证件类型！");
		return false;
	}
	...
	if ((buyerIdCode == null || buyerIdCode == '')
			|| (sellerIdCode == null || sellerIdCode == '')) {
		alert("请填写买/卖方证件编号！");
		return false;
	}
	...
	if ((buyerHomeAddr == null || buyerHomeAddr == '')
			|| (sellerHomeAddr == null || sellerHomeAddr == '')) {
		alert("请填写买/卖方家庭住址！");
		return false;
	}
	...
	var sMortClear = $("input[name='toSpvProperty.isMortClear']").val();
	if (sMortClear == 0) {
		var mortgageeName = $("input[name='toSpvProperty.mortgageeName']")
				.val();
		...
		var mortgageeBank = $("input[name='toSpvProperty.mortgageeBank']")
				.val();
	...
	if ((buyerAccountName == null || buyerAccountName == '')
			|| (sellerAccountName == null || sellerAccountName == '')) {
		alert("请填写买/卖方账户名称！");
		return false;
	}

	var buyerAccountTelephone = $("input[name='toSpvAccountList[0].telephone']")
			.val();
	var sellerAccountTelephone = $(
			"input[name='toSpvAccountList[1].telephone']").val();
	if ((buyerAccountTelephone == null || buyerAccountTelephone == '')
			|| (sellerAccountTelephone == null || sellerAccountTelephone == '')) {
		alert("请填写买/卖方电话！");
		return false;
	}
	...
	/*
	 * var buyerBank =
	 * $("input[name='toSpvAccountList[0].bank']:checked").val(); if(buyerBank ==
	 * null || buyerBank == ''){ alert("请勾选买方开户行！"); return false; }
	 */
...
/*	if ((!new RegExp("^[1-2]\\d{3}-(0?[1-9]||1[0-2])$").test(idValiDate_0) && (idValiDate_0 != null && idValiDate_0 != ''))
			|| (!new RegExp("^[1-2]\\d{3}-(0?[1-9]||1[0-2])$")
					.test(idValiDate_1) && (idValiDate_1 != null && idValiDate_1 != ''))) {
