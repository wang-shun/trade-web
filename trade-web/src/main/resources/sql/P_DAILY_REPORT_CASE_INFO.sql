...
/****** Object:  StoredProcedure [sctrans].[P_DAILY_REPORT_CASE_INFO]    Script Date: 2017/1/19 15:43:25 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	with wf as
	(
		select act_t.ASSIGNEE_,act_t.END_TIME_,act_t.TASK_DEF_KEY_,act_t.PROC_INST_ID_,workflow.CASE_CODE,act_t.DELETE_REASON_ from sctrans.ACT_HI_TASKINST act_t left join sctrans.T_TO_WORKFLOW workflow on workflow.INST_CODE = act_t.PROC_INST_ID_
...
SELECT
		...
		(SELECT REAL_CON_TIME from  sctrans.T_TO_SIGN ts where ts.CASE_CODE=C.CASE_CODE ) AS SIGN_TIME,--签约时间
		...
		(SELECT top (1) ORG_ID  FROM sctrans.SYS_USER  WHERE USERNAME = (SELECT top (1) ASSIGNEE_  FROM wf as ww  WHERE ww.TASK_DEF_KEY_ = 'TransSign' AND ww.CASE_CODE=C.CASE_CODE ORDER BY    END_TIME_ DESC )) as SIGN_TEAM_ID,--签约人组织
		(SELECT DISTRICT_ID  FROM sctrans.v_yucui_org_hierarchy WHERE  TEAM_ID =(SELECT  top (1) ORG_ID  FROM sctrans.SYS_USER  WHERE USERNAME = (SELECT top 1 ASSIGNEE_ FROM  wf as ww WHERE ww.TASK_DEF_KEY_ = 'TransSign' AND ww.CASE_CODE=C.CASE_CODE  ORDER BY  END_TIME_ DESC))) as SIGN_DISTRICT_ID,--签约人贵宾服务部
		...
		(SELECT top (1) ORG_ID FROM sctrans.SYS_USER WHERE USERNAME = ( SELECT top 1 ASSIGNEE_ FROM wf as ww WHERE ww.TASK_DEF_KEY_ = 'Guohu' AND ww.CASE_CODE=C.CASE_CODE ORDER BY END_TIME_ DESC ) ) as HOUSE_TRANFER_TEAM_ID,--过户组别
		(SELECT DISTRICT_ID FROM sctrans.v_yucui_org_hierarchy WHERE TEAM_ID = ( SELECT top (1) ORG_ID FROM sctrans.SYS_USER WHERE USERNAME = ( SELECT top 1 ASSIGNEE_ FROM wf as ww WHERE ww.TASK_DEF_KEY_ = 'Guohu' AND ww.CASE_CODE=C.CASE_CODE ORDER BY END_TIME_ DESC ) ) ) as HOUSE_TRANFER_DISTRICT_ID,--过户贵宾服务部
		...
		(SELECT APPROVE_TIME from  sctrans.t_to_close ts where ts.CASE_CODE=C.CASE_CODE ) AS CLOSE_TIME,--结案时间
		...
		(SELECT top (1) ORG_ID FROM sctrans.SYS_USER WHERE USERNAME = ( SELECT top 1 ASSIGNEE_ FROM wf as ww WHERE ww.TASK_DEF_KEY_ = 'CaseClose' AND ww.CASE_CODE=C.CASE_CODE ORDER BY END_TIME_ DESC )  ) as CLOSE_TEAM_ID,--结案店组
		(SELECT DISTRICT_ID FROM sctrans.v_yucui_org_hierarchy WHERE TEAM_ID = ( SELECT top (1) ORG_ID FROM sctrans.SYS_USER WHERE USERNAME = ( SELECT top 1 ASSIGNEE_ FROM wf as ww WHERE ww.TASK_DEF_KEY_ = 'CaseClose' AND ww.CASE_CODE=C.CASE_CODE ORDER BY END_TIME_ DESC ) )  ) as CLOSE_DISTRICT_ID,--结案贵宾服务部
		...
		FROM
		(select distinct case_code from SCTRANS.T_TO_CASE) C
		INNER join SCTRANS.T_TO_CASE CA on c.CASE_CODE=CA.CASE_CODE
		LEFT JOIN sctrans.T_TO_MORTGAGE MG ON  MG.CASE_CODE=C.CASE_CODE AND MG.IS_ACTIVE=1 AND MG.LAST_LOAN_BANK is not null
		LEFT JOIN SCTRANS.T_TO_CASE_INFO I 			ON C.CASE_CODE = I.CASE_CODE
		LEFT JOIN sctrans.T_TO_HOUSE_TRANSFER TH       ON TH.CASE_CODE=C.CASE_CODE
		where 1=1
