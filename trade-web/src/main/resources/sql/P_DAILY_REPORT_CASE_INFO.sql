		...
		INSERT INTO sctrans.[T_RPT_CASE_BASE_INFO]
           (	
			...
			SELECT  
				...
				(SELECT MAX(END_TIME_) FROM SCTRANS.ACT_HI_TASKINST AT  with(nolock)  LEFT JOIN  SCTRANS.ACT_HI_VARINST AV  with(nolock) ON AT.PARENT_TASK_ID_ = AV.PROC_INST_ID_ AND AV.LONG_ = 1 WHERE AT.TASK_DEF_KEY_=N'GuohuApprove'  AND  AT.DELETE_REASON_ = 'completed' AND AT.PROC_INST_ID_=W.INST_CODE) TRANSFER_LAST_UPDATED_TIME,
